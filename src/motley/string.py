...
class Formatter(string.Formatter):
    ...
    def parse(self, string):
        ...
        # current = 0
        # for match in self.parser.iterate(string, must_close=True, inside_out=False):
        #     #  (literal_text, field_name, format_spec, conversion)
        #     match.full
    ...
    #     try:
    #         return super().format(format_string, *args, **kws)
    #     except ValueError as err:
    #         if 'Single \'}\' encountered' not in str(err):
    #             raise err

    #     braces = [*self.parser.iterate(format_string, condition=(level == 1))]
    #     start = current = braces[0].start
    #     sub = ''
    #     for match in braces:
    #         sub += format_string[current:match.start]
    #         sub += self.format(match.full, *args, **kws)
    #         current = match.end + 1
    #         # logger.debug('{}', sub)

    #     new = ''.join((format_string[:start], sub, format_string[current:]))
    #     # new = format_string.replace(format_string[start:current], '__sub')
    #     logger.debug('{}', new)

    #     # from IPython import embed
    #     # embed(header="Embedded interpreter at 'src/motley/string.py':99")

    #     return self.format(new, *args, **kws)
